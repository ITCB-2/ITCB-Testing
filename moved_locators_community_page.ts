...
export class CommunityMembersSharingPage extends BasePage {
	...
	private readonly locators = CommunityMembersSharingPage._LOCATORS
	...
	async validateCommunityMembersSharingBoxImg(
		...
		await test.step(`Validate ${boxName} Box Image`, async () => {
			const box = findItemByProperty(
				this.locators.communityMembersSharingBoxes,
				'name',
				boxName,
			)
...
type CommunityMemberName =
	(typeof CommunityMembersSharingPage._LOCATORS.communityMembersSharingBoxes)[number]['name']
