...
const COMMUNITY_MEMBERS_SHARING_BOXES = [
	{
		name: 'הילה קדוש',
		img: {
			role: 'img',
...
type CommunityMemberName =
	(typeof COMMUNITY_MEMBERS_SHARING_BOXES)[number]['name']
...
export class CommunityMembersSharingPage extends BasePage {
	...
	private readonly communityMembersSharingBoxes =
		COMMUNITY_MEMBERS_SHARING_BOXES
	...
	async validateCommunityMembersSharingBoxImg(
		...
		await test.step(`Validate ${boxName} Box Image`, async () => {
			const box = findItemByProperty(
				this.communityMembersSharingBoxes,
				'name',
				boxName,
			)
